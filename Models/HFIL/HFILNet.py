...
class HFILNet(nn.Module):
    def __init__(self, norm_layer=nn.LayerNorm):
        ...
        self.rgb_swin = self.load_encoder()
        self.depth_swin = self.load_encoder()
        ...
        self.input_emb_rgb = nn.ModuleList([
            nn.Linear(1024, i) for i in [128, 256, 512, 1024]
        ])
    ...
    def encoder_forward(self, img, depth):
        ...
        rgb_list = self.rgb_swin(img, is_training=True)[::6]
        d_list = self.depth_swin(depth, is_training=True)[::6]
        return rgb_list, d_list

    def forward(self, data):
        rgb = data["image"].permute([0, 3, 1, 2])
        depth = data["depth"].permute([0, 3, 1, 2])

        rgb_list, depth_list = self.encoder_forward(rgb, depth)

        r1 = nn.functional.interpolate(self.input_emb_rgb[3](rgb_list[3]).reshape(-1, 37, 37, 1024).permute(0, 3, 1, 2), 12)
        r2 = nn.functional.interpolate(self.input_emb_rgb[2](rgb_list[2]).reshape(-1, 37, 37, 512).permute(0, 3, 1, 2), 24)
        r3 = nn.functional.interpolate(self.input_emb_rgb[1](rgb_list[1]).reshape(-1, 37, 37, 256).permute(0, 3, 1, 2), 48)
        r4 = nn.functional.interpolate(self.input_emb_rgb[0](rgb_list[0]).reshape(-1, 37, 37, 128).permute(0, 3, 1, 2), 96)

        d1 = nn.functional.interpolate(self.input_emb_depth[3](depth_list[3]).reshape(-1, 37, 37, 1024).permute(0, 3, 1, 2), 12)
        d2 = nn.functional.interpolate(self.input_emb_depth[2](depth_list[2]).reshape(-1, 37, 37, 512).permute(0, 3, 1, 2), 24)
        d3 = nn.functional.interpolate(self.input_emb_depth[1](depth_list[1]).reshape(-1, 37, 37, 256).permute(0, 3, 1, 2), 48)
        d4 = nn.functional.interpolate(self.input_emb_depth[0](depth_list[0]).reshape(-1, 37, 37, 128).permute(0, 3, 1, 2), 96)
        ...
        loss1 = self.loss(pred1, tgt) + self.loss(pred2, tgt) + self.loss(pred3, tgt)
        loss2 = self.iou_loss(pred1, tgt) + self.iou_loss(pred2, tgt) + self.iou_loss(pred3, tgt)
        loss = loss1 + loss2
        return loss.mean(), torch.sigmoid(pred1), tgt, pred2, pred3
