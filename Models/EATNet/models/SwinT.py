...
class WindowAttention(nn.Module):
    ...
    def flops(self, N):
        ...
        flops = 0
        # qkv = self.qkv(input_depth)
        flops += N * self.dim * 3 * self.dim
        # attn = (q @ k.transpose(-2, -1))
        flops += self.num_heads * N * (self.dim // self.num_heads) * N
        #  input_depth = (attn @ v)
        flops += self.num_heads * N * N * (self.dim // self.num_heads)
        # input_depth = self.proj(input_depth)
...
class PatchMerging(nn.Module):
    ...
    def forward(self, x):
        ...
        B, L, C = x.shape
        assert L == H * W, "input feature has wrong size"
        assert H % 2 == 0 and W % 2 == 0, f"input_depth size ({H}*{W}) are not even."
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=384,
                 ...
                 # TODO
                 # len(depths)代表了swin-transformer有几个stage，
                 # 而depths中的每一个值代表每一个stage中有多少个Swin Transformer Block
                 # 在具体的算法中，下面的参数被设置为[2，2，18，2]
                 depths=[2, 2, 6, 2],

                 # num_heads指的是在每一个stage中的所有Block中的MSA使用的头数，由默认值我们可以看到第一个stage中的MSA的头数是3个
                 # 在实现的模型中，下面的参数修改了设置
                 num_heads=[3, 6, 12, 24],
    ...
    def forward_features(self, x):
        ...
        for layer in self.layers:
            x = layer(x)
            B, L, C = x.shape
            # print('input_depth:', input_depth.shape)
            xl = x.view(B, int(np.sqrt(L)), int(np.sqrt(L)), -1).permute(0, 3, 1, 2).contiguous()
            # print('xl',xl.shape)
            layer_features.append(xl)  # TODO 添加特征

        x = self.norm(x)  # B L C
        B, L, C = x.shape

        # TODO 添加特征
        # input_depth = self.avgpool(input_depth.ranspose(1, 2))  # B C 1
        x = x.view(B, int(np.sqrt(L)), int(np.sqrt(L)), -1).permute(0, 3, 1, 2).contiguous()
        # input_depth = torch.flatten(input_depth, 1)
        layer_features[-1] = x
